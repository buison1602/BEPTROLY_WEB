...
import { dietNoteService } from "~/features/users/api/dietNoteService"; // Đảm bảo bạn đã tạo service này
...
export default function ProfilePage() {
  ...
  const [dietNotes, setDietNotes] = useState([]); // State cho ghi chú ăn uống
  ...
  const handleUpdateInfo = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await userService.updateInfo({ userId: parseInt(userId!), ...user });
    if (res.success) {
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                ...
                <form onSubmit={handleUpdateInfo} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" value={user.fullName} onChange={e => setUser({...user, fullName: e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl outline-none focus:ring-2 focus:ring-[#f59127cc]" placeholder="Họ và tên"/>
                    <input type="email" value={user.email} onChange={e => setUser({...user, email: e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl outline-none focus:ring-2 focus:ring-[#f59127cc]" placeholder="Email"/>
                  </div>
               ...
               <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                  ...
                  <form onSubmit={handleAddDietNote} className="flex flex-col md:flex-row gap-3 mb-10 p-4 bg-orange-50 rounded-[2rem]">
                    <select 
                      ...
                      className="p-4 bg-white rounded-2xl outline-none font-bold text-sm text-[#f59127] min-w-[150px]"
                    >
                    ...
                    <input 
                      ...
                      placeholder="Ví dụ: Tôm, Đậu phộng, Không ăn cay..."
                      className="flex-1 p-4 bg-white rounded-2xl outline-none font-medium"
                    />
                    <button type="submit" className="p-4 bg-[#f59127] text-white rounded-2xl font-black hover:scale-105 transition-transform shadow-md">
                      <Plus />
                    </button>
                  </form>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {dietNotes.length > 0 ? dietNotes.map((note: any) => (
                      <div 
                        key={note.noteId} 
                        className={`flex items-center gap-3 px-6 py-3 rounded-full border-2 transition-all font-bold group ${
                          note.noteType === 'allergy' ? 'bg-red-50 border-red-100 text-red-600' :
                          note.noteType === 'restriction' ? 'bg-orange-50 border-orange-100 text-orange-600' :
                          'bg-green-50 border-green-100 text-green-600'
                        }`}
                      >
                        <span className="text-[10px] uppercase opacity-60 tracking-tighter">{note.noteType}</span>
                        <span>{note.label}</span>
                        <button onClick={() => handleDeleteDietNote(note.noteId)} className="hover:scale-125 text-gray-400 hover:text-red-500 transition-all">
                          <X size={16} />
                        </button>
                      </div>
                    )) : (
